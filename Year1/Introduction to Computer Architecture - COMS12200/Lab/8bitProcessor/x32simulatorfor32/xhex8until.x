...
{ if (symbol = s_skip)
  ...
  { nextsymbol();
    ...
    checkfor(s_do, "\'do\' expected");
    b := rstatement();
    return cons3(s_while, cons2(s_not,a), b)
  }
  else  
  if (symbol = s_begin)
  then
...
{ bufferp := 0; 
  ...
  { cb_unpack(bufferp);
    ...
    if flag = cbf_constp
    then
    { cb_conststart := div(cb_loadpoint, bpw);
      cb_stringstart := cb_conststart + constp;
      cb_loadpoint := cb_loadpoint + mul2(constp + stringp, bpw)
    }
    else
    if flag = cbf_entry 
    then
